...
- S-FREE-3's merged local platform code adds JWT-owner-bound free products, Check history, and reported experiences behind an Edge-only API. Manual product text and reaction notes are private user reports, not catalog or diagnosis evidence. S-FREE-4's merged local private product-photo path does not enable hosted guest signup or mobile capture wiring.
