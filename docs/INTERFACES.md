...
- **Centralized Coordinator**: All UI routes (`app/**`) execute domain mutations and queries exclusively through `src/services/deriveClient.ts`:
  ...
  - `submitWeeklyCheckIn(input)`: Records longitudinal observation and syncs store cache. Current `CheckInInput` has `notes` but no persisted context tags. I1-B4B will add optional `contextTags` + `contextNote` (not implemented).
