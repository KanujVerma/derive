...
→ K2/S2 live personalization integration (#63) ✅; K3/S3 live MY STUFF integration (#64) ✅
→ K4/S4 camera-to-evidence integration (#65) ✅ (local); S-OPS-1 before hosted guest activation
```
...
- S6 provides product identity and formula evidence states. Unknown identity/formula stays unknown. Camera capture is not proof of identity; visual/OCR provider behavior remains unproven. Formula Details stays separate from Personal Fit; K2/S2 shows categorical fit only when the evidence supports it.
...
- S-FREE-2 is merged into main as a local platform contract, separate from paid intake. K2/S2 live integration (#63) now maps the presentation model to the typed profile and deterministic fit services. Hosted deployment and hosted guest activation remain gated; see [S_FREE_2_PERSONAL_FIT.md](S_FREE_2_PERSONAL_FIT.md).
- S-FREE-3 is merged into main as a local platform contract: owner-bound saved products, Check history, and reported experiences for anonymous/permanent free identities. K3/S3 live integration (#64) connects the MY STUFF projection and persistence. Hosted deployment and guest activation remain gated; see [S_FREE_3_CONTEXT_HISTORY.md](S_FREE_3_CONTEXT_HISTORY.md).
- S-FREE-4 is merged into main as a local platform contract: server-granted private free product photos, S6 candidate/unknown resolution, and paid Shelf separation. K4/S4 integration (#65) connects camera evidence to S6 locally; no hosted deployment or OCR provider is claimed. A client-side gap remains: prepare-evidence HTTP 429 quota errors collapse to generic `PREPARE_FAILED` instead of a typed quota state. The backend's 429 response is unchanged. See [S_FREE_4_PRODUCT_EVIDENCE.md](S_FREE_4_PRODUCT_EVIDENCE.md).
...
Landed platform checkpoints: S-FREE-2 PR #54 at `5f764d7715ca6ba0a4a585ee46c5f13dc78dc855`; S-FREE-3 PR #58 at `fad55ce4fd38a91fa549c86bda1a8fc77c9e569c`; S-FREE-4 PR #61 at `cfdbf97b410b8c64d007e819a2a7155c9e3613a7`. K2/S2, K3/S3, and K4/S4 local integrations are merged in PRs #63, #64, and #65. Exact-head and post-merge CI evidence is recorded in [CONTEXT_SYNC.md](CONTEXT_SYNC.md). The Retinyl Propionate regression has tests for pregnancy, unknown context, and no-positive-fit behavior. Hosted deployment did not occur.
...
| 2, platform/presentation and live integration landed | **K-FREE-2 Progressive Personalization UX ✅ (PR #51); K2/S2 integration ✅ (PR #63).** | **S-FREE-2 Minimal Profile + Deterministic Personal Fit ✅ (PR #54).** | Typed profile save/read and same-result fit refresh are connected in the local app; hosted deployment remains gated. |
| 3, platform/presentation and live integration landed | **K-FREE-3 MY STUFF ✅ (PR #48); K3/S3 integration ✅ (PR #64).** | **S-FREE-3 Free Context / Check History Data Plane ✅ (PR #58):** owner-bound persistence for anonymous and permanent identities. | Local MY STUFF reads/writes use owner-bound persistence; hosted deployment remains gated. |
| 4, local integration landed | **K-FREE-4 Camera Product Evidence UX ✅ (PR #49); K4/S4 ✅ (PR #65).** | **S-FREE-4 Product Evidence Resolution ✅ (PR #61):** private evidence, S6 resolution, candidate/uncertainty states. | Camera-to-resolver integration is merged locally. Typed client handling for the backend 429 quota response remains open; no hosted deployment or OCR provider is claimed. |
