...
* **Shared resolver**: One future Product Identity Resolver serves both Scan and onboarding Shelf. Camera/user evidence flows through extraction, candidate retrieval, identity resolution, formula verification, user confirmation when needed, then personalized Scan evaluation. Barcode/GTIN, front-label OCR, brand/name/variant, packaging image, ingredient OCR, user text, catalog, merchant listing identity and authoritative external product sources are candidate evidence. Merchant economics never affect identity or skincare verdicts.
...
### C1.5A: Multi-Merchant Acquisition Foundation [DRAFT PR]
* **Scope**: Shop-owned exact identity resolver, curated verified merchant listings, optional offer presentation and member Where to Buy external links. Product/recommendation truth remains upstream; provisional products and unpublished ADD fail closed. No backend migration, live price, checkout or public routing.

### C1.5B: Official Retailer Feeds, Live Offers & Attribution [PLANNED]
* **Scope**: Verify official merchant APIs/feeds or approved networks, then ingest live prices, availability, identifiers, freshness and approved affiliate attribution with provenance. HTML scraping is not the core data source. No C1.5B code in A.
