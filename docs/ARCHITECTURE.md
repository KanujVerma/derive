...
* **Mobile Auth & Session Spine (`src/services/authClient.ts`, `src/stores/authStore.ts`)**:
  ...
  - **Deterministic Route Gating**:
    ...
    - Remote Mode (`EXPO_PUBLIC_USE_REMOTE_SERVICE=true`): `INITIALIZING` displays minimal Mineral loading splash; `SIGNED_OUT` routes to `/(auth)/login`; `SIGNED_IN` routes into a neutral profile-resolution holding state (`/holding`) while canonical remote profile hydration (I1-A2) determines onboarding and membership status. Local onboarding state is never consulted.
