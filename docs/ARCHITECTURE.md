...
* **Membership vs products**: Membership does not include products. C1 provides a five-tab member Shop with Scan inside Shop. C1.5A composes current published recommendations and trusted catalog identity to stable merchant listings and optional provenance-backed offer snapshots in the Shop client. One verified external retailer page exists for one product; this is product-family acquisition, not formula verification. No live offers, product checkout, product order schema or backend migration is included. C1.5B owns official feeds; C1.5C plans Derive as a Shopify merchant.
...
## 7. S6 planned: one Product Identity Resolver for Scan and Shelf
...
S6 belongs to Sami/backend-platform and has no implementation in C1.5A. The target shared resolver accepts camera/user evidence (GTIN, label OCR, brand/name/variant, packaging, ingredient OCR, typed name), catalog facts, merchant listing identifiers and authoritative sources. It extracts evidence, retrieves candidates, resolves identity, verifies a formula against provenance and `FormulaSnapshot`, asks the user to choose a variant or photograph ingredients when needed, and only then passes supported facts to personalized Scan evaluation or Shelf. Vision/model resemblance proposes candidates; it does not establish canonical identity. Conceptual outcomes are verified product, identified but formula unverified, ambiguous candidates, formula only and insufficient evidence. Formula provenance accounts for reformulation, region, variant and older packaging. This one resolver serves both Scan and Shelf; merchant commission and offers never become evidence of product truth or fit.
