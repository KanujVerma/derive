...
* **Authenticated Remote Onboarding Intake & Private Photo Pipeline (I1-B1 Implemented)**:
  - **Internal Submission Ledger (`public.onboarding_submissions`)**: Stores server-issued storage paths, status (`draft` | `committed`), and sanitized intake snapshots (`payload_snapshot JSONB`) with a partial unique index (`WHERE status = 'draft'`) guaranteeing a single active draft per member. RLS is enabled and all direct access is revoked from `anon` and `authenticated` roles, isolating staging state to trusted `service_role` operations.
  - **Review Task Idempotency**: `public.founder_review_tasks` enforces a partial unique index on `(user_id, task_type) WHERE task_type = 'initial_routine' AND status = 'pending'`, preventing duplicate pending initial review tasks across intake retries.
  ...
  - **Private Photo Upload Pipeline (`src/services/onboardingPhotoUpload.ts`)**: Direct upload to `customer-skin-photos` at server-issued paths with `upsert: false`. Detects MIME types and throws on error, never persisting local URIs.
  - **Contract & Client Flow**: Evolved `OnboardingResult` with `proposedRoutine: Routine | null` and `initialRoutineState: InitialRoutineState` (`'pending_generation' | 'awaiting_review'`). In Remote mode, `app/(onboarding)/10-summary.tsx` re-resolves `CustomerBootstrapState` and verifies `onboardingCompleted === true` before routing to `/(tabs)`. Routine generation is intentionally deferred to I1-B2.
