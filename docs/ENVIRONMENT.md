...
The L0 resolver rejects invalid flavor values, Remote staging with Remote disabled, missing URL/key, legacy anon-only key, local/non-HTTPS or non-`*.supabase.co` hosts, and URLs containing a path, query, fragment, or embedded credentials. A future reviewed custom-domain rollout would need an explicit guard change. The build shows a staging-only identity card on sign-in, Membership, and Profile with its service mode, safe backend host, configuration status, and app version. It never shows a key or user token. H1A must compare the host on a built device with the verified project; a TestFlight label alone is not evidence of Remote mode.
