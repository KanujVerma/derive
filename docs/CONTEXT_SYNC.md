...
- **Physical boundary:** Direct USB CoreDevice service connected to the paired, booted iPhone 17 Pro Max on iOS 27.0 with Developer Mode enabled. Build `4` cold-launched and USB terminate/relaunched without native crash, routed to canonical signed-out login, showed the correct Remote staging host/mode/configuration shape and preserved safe invalid-email state across background/foreground. Process restart cleared that local draft/error. Camera, barcode and face-capture runtime remain legitimately Auth-gated by H1E. Network-offline mutation and hosted OTP request require their own action-time approval/evidence. [L1A_DEVICE_PREFLIGHT.md](L1A_DEVICE_PREFLIGHT.md) holds the matrix.
- **Binary and defect boundary:** private IPA inspection confirmed `com.derive.skincare` `1.0.0 (4)`, the staging diagnostic marker, exact Derive host and native camera/module strings, with no Gemini or Stripe secret-shaped value. Direct USB install failed closed at Apple's store boundary as expected, so TestFlight performed the replacement. Physical diagnostics exposed one Kanuj-owned defect: version `1.0.0` appeared but build number `4` was missing because `expo-constants` returned no immutable native build. L1A now reads `expo-application` native version/build under a failing-then-passing regression guard; replacement binary verification remains required.
