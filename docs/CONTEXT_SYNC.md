...
- **Verification:** fresh full migration reset passed; all 11 pgTAP files passed (**341 assertions**); unit suite passed (**275 tests** after final resolver, endpoint, and Scan integration coverage); app and test TypeScript checks passed; the Expo web export passed; Edge Runtime booted all four affected functions (`resolve-product-identity`, `scan-product`, `founder-operations`, and `delete-customer-account`) and each reached its authentication guard; diff, secret-pattern, scope, and final upstream checks passed against unchanged `origin/main@556a43e0b70f50264c459ae476e2214061a8876a`.
