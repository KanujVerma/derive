...
- **Verification:** after reconciliation, a fresh full migration reset applied all 17 migrations through Build 9; all 12 pgTAP files passed (**364 assertions**); the combined unit suite passed (**310 tests**, including Build 10); app and test TypeScript checks and the Expo web export passed; the resolver booted under Edge Runtime and reached its authentication guard. Final exact-head CI remains the closure gate after commit.
