...
- **Main CI incident:** Verify & Build passed, but three main Database & Integration attempts stopped before application assertions because GHCR pulls returned `toomanyrequests`, first for `pg_prove` and then for Supabase stack images. An authenticated GHCR pull also hit the same registry limit on the follow-up PR. The follow-up now selects the CLI-supported Docker Hub registry for Supabase images, pending exact-head CI. This does not change application, database, or hosted state. Main CI still needs fresh verification after a working repair lands.
