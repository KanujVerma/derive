...
- **Verification:** after reconciling the completed L1A checkpoint at `origin/main@7a3da300ba6ad78875f225969cf3457e24aa07d7`, a fresh full migration reset passed; all 11 pgTAP files passed (**341 assertions**); the combined unit suite passed (**276 tests**); app and test TypeScript checks and the Expo web export passed; Edge Runtime booted all four affected functions (`resolve-product-identity`, `scan-product`, `founder-operations`, and `delete-customer-account`) and each reached its authentication guard; diff, secret-pattern, scope, and upstream checks passed.
