...
- **L1A complete**: store-signed Remote staging `1.0.0 (5)` is installed and physically verified on an iPhone 17 Pro Max. Signed-out launch, identity diagnostics, validation, restart, background/foreground and offline/recovery passed. Authenticated/customer camera paths were not bypassed and remain H1E-dependent.
- **Next work**: Sami owns F1 manual routine fallback now; H1P model-provider activation, H1E email and H1B Stripe are separate platform gates. Kanuj starts L1B only after F1/H1E and starts L1C only after H1P/H1E. H1A's password-auth scripts never authorize a mobile Auth bypass. See `docs/ROADMAP.md` and `docs/OWNERSHIP.md`.
