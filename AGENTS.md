...
- **Next parallel work**: Kanuj owns L1A staging device preflight and later customer acceptance. Sami resumes platform ownership for F1 manual routine fallback; H1E email and H1B Stripe are separate later activation milestones. L1A cannot pass routine/member acceptance until a publishable routine path exists. See `docs/ROADMAP.md` and `docs/OWNERSHIP.md`.
