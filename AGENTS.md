...
- **L0 complete**: the `remote-staging` EAS profile selects preview public configuration and Remote mode, while development and production remain Mock. Build flavor validation fails closed and staging-only diagnostics show safe build identity. EAS preview hosted URL/key still require H1A project verification before a real staging build.
- **H1A next**: founder authorization assigns this one hosted-core platform milestone to Kanuj on a fresh branch after L0 lands. H1E email and H1B Stripe activation remain separate Sami work. Normal platform ownership returns to Sami after H1A. See `docs/ROADMAP.md`.
