...
- **H1 hosted state**: Existing Derive Supabase project `snojlbqovlawewwqbviz` has all 15 migrations and 13 functions deployed. The real OTP, provider, Stripe test, and customer lifecycle smoke remain pending. Read `docs/HOSTED_REMOTE_SMOKE.md` before any hosted change; never reset the linked project or enable production Remote by default.
