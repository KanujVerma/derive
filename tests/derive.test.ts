...
test('S3 endpoint guard: replay precedes Gemini and founder notes stay server-internal', () => {
  ...
  const replayLookup = proposeRoutine.indexOf('loadExistingInitialRoutineId(admin, userId)');
  const modelCall = proposeRoutine.indexOf('generateStructuredJson(');

  assert.ok(replayLookup >= 0, 'Routine endpoint must check for a replayable initial routine');
  assert.ok(modelCall > replayLookup, 'Replay lookup must occur before any Gemini request');
