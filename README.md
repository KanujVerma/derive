...
Managed skincare service ("Your skincare, handled"). Built on Apple-grade minimalism, contextual intelligence, and persistent customer care. Currently preparing for an initial 10-member Founding Beta cohort. **Approved commercial direction (not yet in code):** $25/month membership for Derive managing skincare, with products purchased separately (ADR-26 / I1-B4A). Current client still displays `$100/mo` via `config.betaPriceMonthly`.
...
- **Commerce**: Planned Stripe web checkout for Founding Beta **membership** (approved $25/month after I1-B4A; Stripe in S5). Products are separate transactions. Full Shop is deferred. Current client still shows `$100/mo`.
